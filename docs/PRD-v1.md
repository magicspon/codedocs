> **Frozen.** This is the original product requirements document, written before any code existed
> and superseded by [`docs/REQUIREMENTS.md`](REQUIREMENTS.md). It is kept unedited because ADRs
> 0001–0011 cite it by section number — every `PRD §N` reference in `docs/adr/` resolves here, and
> a citation records what a decision was taken against. Nothing in it is a statement of current
> intent; where it and the ADRs disagree, the ADRs win.
>
> The reframing is [ADR 0012](adr/0012-audience-and-the-fallow-boundary.md).
