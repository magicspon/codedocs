...
const LABEL_SELECT = `select p.path, n.qualified, l.axis, l.value, l.provenance,
    l.derivation
  from label l
...
const toLabel = (row: LabelRow): Label => ({
  node: idOf(row.path, row.qualified),
...
export function readLabels(store: Store): Label[] {
  return (
    store.db
      .prepare(`${LABEL_SELECT} order by p.path, n.qualified, l.axis`)
      .all() as LabelRow[]
  ).map(toLabel)
}
